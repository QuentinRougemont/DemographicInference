...
if [ $folded = "folded" ]
then
	python ../02-modifs/folded/script_inference_anneal2_newton_mis_new_models_folded2.py -o "$model"_"$id" -y pop1 -x pop2 -p $grid_size,$grid_size2,$grid_size3  -f "$fs" -m "$model" -l -v  -z &>> ../10-log/"$model"_"$id".log;
else
