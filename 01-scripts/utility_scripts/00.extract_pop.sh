...
vcftools --vcf "$vcffile" --keep "$pop1" --max-missing 0.9 \
	--out "$folder"/batch_"$pop1" --recode
vcftools --vcf "$vcffile" --keep "$pop2" --max-missing 0.9  \
...
# Merge VCF 
vcf-merge "$folder"/batch_"$pop1".recode.vcf.gz \
	"$folder"/batch_"$pop2".recode.vcf.gz > "$folder"/batch_"$pop1"."$pop2".vcf

# Keeps SNPs in the 2 pops
grep -v "^#" "$folder"/batch_"$pop1"."$pop2".vcf |\
	grep -e "0,1" > "$folder"/batch_"$pop1"."$pop2".spectrum.tmp 
...
less "$folder"/freq_"$pop1"."$pop2".frq |grep -v ":1" |\
	awk '{print $1, $2}' |\
	sed 's/ /\t/g' > "$folder"/polymorphic_sites 
vcftools --vcf  "$folder"/batch_"$pop1"."$pop2".spectrum.vcf \
	--positions "$folder"/polymorphic_sites \
	--out "$folder"/batch_"$pop1"."$pop2".spectrum.polym  \
...
vcftools --vcf "$folder"/batch_"$pop1"."$pop2".spectrum.polym.recode.vcf \
	--keep "$pop1" --out "$folder"/batch_"$pop1"_spectrum --recode
vcftools --vcf "$folder"/batch_"$pop1"."$pop2".spectrum.polym.recode.vcf \
	--keep "$pop2" --out "$folder"/batch_"$pop2"_spectrum --recode
...
paste "$folder"/batch_"$pop1"_spectrum.frq "$folder"/batch_"$pop2"_spectrum.frq |\
	perl -pe 's/:/\t/g' |\
	sed '1d' > "$folder"/batch_spectrum.txt
